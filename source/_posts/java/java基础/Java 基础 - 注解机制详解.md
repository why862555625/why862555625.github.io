...
tags:
  ...
  - java基础
categories: java
keywords: Java 基础 - 注解机制详解
description: Java 基础 - 注解机制详解
...
### [#](#java8提供了哪些新的注解) Java8 提供了哪些新的注解？
...
### [#](#继承实现到注解实现-junit3到junit4) 继承实现到注解实现 - Junit3 到 Junit4

> 一个模块的封装大多数人都是通过继承和组合等模式来实现的，但是如果结合注解将可以极大程度提高实现的优雅度（降低耦合度）。而 Junit3 到 Junit4 的演化就是最好的一个例子。
...
通过定义@Before，@Test，@After 等等注解来实现。
...
这里我们发现通过注解的方式，我们实现单元测试时将更为优雅。如果你还期望了解 Junit4 是如何实现运行的呢？可以看这篇文章：[JUnit4 源码分析运行原理在新窗口打开](https://blog.csdn.net/weixin_34043301/article/details/91799261)。

### [#](#自定义注解和aop-通过切面实现解耦) 自定义注解和 AOP - 通过切面实现解耦

> 最为常见的就是使用 Spring AOP 切面实现**统一的操作日志管理**，我这里找了一个开源项目中的例子（只展示主要代码），给你展示下如何通过注解实现解耦的。
...
以一个简单的 CRUD 操作为例, 这里展示部分代码：每对“部门”进行操作就会产生一条操作日志存入数据库。
